...
public class HotelBookingController {
    ...
    /**
     * 根据旅游订单ID查询所有酒店预订（支持多酒店）
     */
    @GetMapping("/tour-booking/{tourBookingId}")
    @ApiOperation("根据旅游订单ID查询所有酒店预订")
    public Result<List<HotelBookingVO>> getByTourBookingId(@PathVariable Integer tourBookingId) {
        log.info("根据旅游订单ID查询所有酒店预订：{}", tourBookingId);
        List<HotelBookingVO> hotelBookings = hotelBookingService.getByTourBookingIdWithDetails(tourBookingId);
        return Result.success(hotelBookings);
    }
     ...
     */
    @DeleteMapping("/batch")
    @ApiOperation("批量删除酒店预订")
    public Result<String> batchDeleteBookings(@RequestBody java.util.Map<String, java.util.List<Integer>> request) {
        java.util.List<Integer> ids = request.get("ids");
        log.info("批量删除酒店预订，IDs：{}", ids);
        ...
        try {
            ...
            if (successCount == ids.size()) {
                ...
                return Result.success("部分删除成功，成功删除 " + successCount + "/" + ids.size() + " 条记录");
            }
        } catch (Exception e) {
