...
public class EmailAsyncServiceImpl implements EmailAsyncService {
    ...
    /**
     * 🚀 订单确认后异步发送确认邮件（不阻塞主线程）
     * @param emailConfirmationDTO 确认邮件信息
     */
    @Override
    @Async("emailTaskExecutor")
    public void sendConfirmationEmailAsync(EmailConfirmationDTO emailConfirmationDTO) {
        log.info("🚀 异步确认邮件发送任务开始: orderId={}, 线程={}", 
                emailConfirmationDTO.getOrderId(), Thread.currentThread().getName());
        
        try {
            // 调用同步邮件服务发送确认邮件
            emailService.sendConfirmationEmail(emailConfirmationDTO);
            log.info("✅ 异步确认邮件发送成功: orderId={}", emailConfirmationDTO.getOrderId());
        } catch (Exception e) {
            log.error("❌ 异步确认邮件发送失败: orderId={}, error={}", 
                    emailConfirmationDTO.getOrderId(), e.getMessage(), e);
            
            // 邮件发送失败不影响订单确认流程，只记录错误日志
            // 后续可以添加重试机制或者人工处理
        }
    }
} 