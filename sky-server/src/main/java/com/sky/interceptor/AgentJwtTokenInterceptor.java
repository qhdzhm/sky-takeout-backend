...
public class AgentJwtTokenInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        // 检查是否是登录请求
        if (requestURI.contains("/agent/login")) {
            log.debug("代理商登录请求，直接放行: {}", requestURI);
            return true;
        }
        ...
        if (log.isDebugEnabled()) {
            log.debug("代理商jwt校验:{}", token);
        }

        if (token == null) {
            //未携带token，不通过，响应401状态码
            log.debug("代理商jwt校验:token为空");
        ...
        try {
            ...
            //降低日志级别，避免大量错误日志
            log.debug("代理商jwt校验异常: {}", ex.getMessage());
            //4、不通过，响应401状态码
            response.setStatus(401);
            return false;
        }
