...
public interface ServiceSessionMapper {
     ...
     * 插入新会话
     */
    @Insert("insert into service_session (session_no, user_id, user_type, session_status, session_type, subject, start_time, create_time, update_time) " +
            "values (#{sessionNo}, #{userId}, #{userType}, #{sessionStatus}, #{sessionType}, #{subject}, #{startTime}, #{createTime}, #{updateTime})")
     ...
     * 获取等待分配的会话列表（带完整用户信息）
     */
    List<ServiceSessionVO> getWaitingAssignSessions();
