...
public interface VehicleMapper {
     ...
     * 根据ID查询车辆
     */
    @Select("SELECT vehicle_id, vehicle_type, license_plate, acn, maker, model, year, " +
            "rego_expiry_date, inspection_due_date, status, notes, max_drivers, location, " +
            "seat_count, create_time, update_time, fuel_level, current_location " +
     ...
     * 新增车辆
     */
    @Insert("INSERT INTO vehicles (vehicle_type, license_plate, acn, maker, model, year, " +
            "rego_expiry_date, inspection_due_date, status, notes, max_drivers, location, " +
            "seat_count, create_time, update_time) " +
            "VALUES (#{vehicleType}, #{licensePlate}, #{acn}, #{maker}, #{model}, #{year}, " +
            "#{regoExpiryDate}, #{inspectionDueDate}, #{status}, #{notes}, #{maxDrivers}, " +
            "#{location}, #{seatCount}, #{createTime}, #{updateTime})")
    ...
    @Update("UPDATE vehicles SET vehicle_type = #{vehicleType}, license_plate = #{licensePlate}, " +
            "acn = #{acn}, maker = #{maker}, model = #{model}, year = #{year}, " +
    ...
    @Select("SELECT vehicle_id, vehicle_type, license_plate, acn, maker, model, year, " +
            "rego_expiry_date, inspection_due_date, status, notes, max_drivers, location, " +
            "seat_count, create_time, update_time, fuel_level, current_location " +
            "FROM vehicles WHERE license_plate = #{licensePlate}")
    ...
    @Select("SELECT vehicle_id, vehicle_type, license_plate, acn, maker, model, year, " +
            "rego_expiry_date, inspection_due_date, status, notes, max_drivers, location, " +
            "seat_count, create_time, update_time, fuel_level, current_location " +
            "FROM vehicles WHERE status = 1 ORDER BY vehicle_id")
